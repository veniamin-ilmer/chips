  ...
  fn type_2(&mut self, operation_code: u5, mut word_select: WordSelect) {
    ...
    for i in 0..14 {
      ...
      if word_select.read_and_shift_bit(direction, false) {
        //This is meAnR for math calculations, where we only need the first digit to be one.
    ...
    match instruction.value() & 0b11 {
      ...
      0b10 | 0b11 => {
        match instruction.value() >> 2 {
          ...
          0b1110 => { trace!("CLEAR REGS"); *self = AnR::new(); },
          _ => unimplemented!(),
        }
      },
      _ => todo!(),
